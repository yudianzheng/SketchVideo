...
class Loss(nn.Module):
    def __init__(self, args):
        ...
        self.clip_text_weight = args.clip_text_weight
    ...
    def forward(self, sketches, targets, text, color_parameters, renderer, epoch, points_optim=None, mode="train"):
        loss = 0
...
class CLIPTextEncoder(nn.Module):
    ...
    def forward(self, x):
        fc_features = self.clip_model.encode_text(x).float()
        return fc_features
...
class CLIPConvLoss(torch.nn.Module):
    ...
    def forward(self, sketch, target=None, text=None, mode="train"):
        """
        Parameters
        ----------
        ...
        if target != None:
            img_augs = [self.normalize_transform(y)]  # 1 list of 49x3x224x225
        if text != None:
            template = get_augmentations_template() # 14 template
            text_token = clip.tokenize(random.choices(compose_text_with_templates(text, template), k=(self.num_augs+1)*len(sketch))).to(self.device)
        if mode == "train":
            for n in range(self.num_augs):
                if target != None:
                    augmented_pair = self.augment_trans(torch.cat([x, y]))
                    sketch_augs.append(augmented_pair[0:len(x)])
                    img_augs.append(augmented_pair[len(x):])
                else:
                    sketch_augs.append(self.augment_trans(x))

        xs = torch.cat(sketch_augs, dim=0).to(self.device)  # 245 x 3x224x224
        if target != None:
        ...
        if self.clip_model_name.startswith("RN"):
            ...
            xs_fc_features, xs_conv_features = self.visual_encoder(xs) # 245x512; list of 12
            ys_fc_features, ys_conv_features = self.visual_encoder(ys)

        if text != None:
            text_fc_features = self.text_encoder(text_token) # 14x512


        if target != None:
            conv_loss = self.distance_metrics[self.clip_conv_loss_type](   # list of 12
                xs_conv_features, ys_conv_features, self.clip_model_name)

            for layer, w in enumerate(self.args.clip_conv_layer_weights): # can be changed, nth layer
                if w:
        ...
        if text != None:
            if self.clip_text_weight:
                text_fc_loss = (1 - torch.cosine_similarity(xs_fc_features,
                        text_fc_features.detach(), dim=1)).mean()
                conv_loss_dict["clip_text"] = text_fc_loss * self.clip_text_weight
